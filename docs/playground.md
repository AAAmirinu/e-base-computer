...
- **Official Challenge**: 公式スイートの正解判定、総合点、提出用JSON。
- **Thermal Timeline**: 命令ごとの最大温度推移。
...
コンテスト提出には、CLIまたはローカルサーバ版 `Run Official Suite` の `Copy JSON` を使ってください。

## 公式チャレンジ

Playgroundの `Run Official Suite` は、CLIの次のコマンドと同じ公式スイートを実行します。
...
サーバAPIとしては、`/api/challenge` と `/api/challenge?name=thermal-degrade` も使えます。

静的版では `/api/challenge` が存在しないため、`Run Official Suite` は `static fallback` の内蔵スイートを実行します。
これは公開ページで雰囲気を伝えるための軽量版です。静的版では `Copy JSON` は無効になり、結果JSONにも
